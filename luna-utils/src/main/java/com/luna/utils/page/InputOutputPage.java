...
public class InputOutputPage implements IInputOutput<PageInput, PageOutput> {
	...
	public PageOutput get(PageInput input) {
		...
		// 结束迭代计算公式
		long endCalc = iteratorStart + (input.getIteratorCount() - 1);
		// 结束迭代值
		long iteratorEnd = endCalc > pages ? pages : endCalc;

		long lastDiff = pages - input.getTotalCount();

		if (input.getPageNow() > lastDiff) {
			iteratorEnd = pages;
			startCalc = iteratorEnd - input.getIteratorCount() + 1;
			iteratorStart = startCalc < 1 ? 1 : startCalc;
		}
